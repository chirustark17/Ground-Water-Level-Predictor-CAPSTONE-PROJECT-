...
def train_models():
    ...
    preds_xgb = xgb.predict(X_encoded)

    for name, preds in [("Random Forest", preds_rf), ("XGBoost", preds_xgb)]:
        rmse = np.sqrt(mean_squared_error(y, preds))
        r2 = r2_score(y, preds)
        accuracy = max(0, (1 - rmse / np.mean(y)) * 100)
    ...
    return rf, xgb, X_encoded.columns, metrics


rf_model, xgb_model, trained_columns, model_metrics = train_models()
                ...
                if selection_df.empty:
                    ...
                    with col1:
                        ...
                        st.pydeck_chart(pdk.Deck(
                            map_style=map_styles[map_theme],
                            initial_view_state=view_state,
                            layers=[heatmap, scatter],
                            tooltip={"html":"<b>Village:</b> {VILLAGE_NA}<br/><b>WL:</b> {WL(mbgl)} mbgl"}
                        ))
                    ...
                    with col2:
                        ...
                        model_used = rf_model if model_choice == "Random Forest" else xgb_model
                        ...
                        pred = model_used.predict(input_aligned)[0]

                        st.metric(f"{model_choice} Predicted WL", f"{pred:.2f} mbgl",
                                  delta=f"{pred - kpi_data['WL(mbgl)']:.2f} m vs Actual")
                        ...
                        if st.button("Predict"):
                            custom_df = pd.DataFrame([{
                                'Rainfall(mm/Year)': rainfall_input,
                                'Block Population': pop_input,
                                'Predominant_Soil_Type': soil_input,
                                'lat': lat_input,
                                'lon': lon_input
                            }])
                            ...
                            custom_pred = model_used.predict(custom_aligned)[0]
                            st.success(f"{model_choice} Prediction: {custom_pred:.2f} mbgl")

                            st.components.v1.html(
                                create_soil_visualization(soil_input, custom_pred),
                                height=370
                            )
                        ...
                        colA.metric("RMSE", f"{selected_metrics['RMSE']:.3f}")
                        colB.metric("R² Score", f"{selected_metrics['R2']:.3f}")
                        colC.metric("Accuracy", f"{selected_metrics['Accuracy']:.1f}%")

                    st.divider()

                    # --- Graphs + Soil Visualization ---
                    ...
                    with graph_col2:
                        ...
                        st.components.v1.html(
                            create_soil_visualization(
                                kpi_data['Predominant_Soil_Type'],
                                kpi_data['WL(mbgl)']
                            ),
                            height=370
                        )
